...
<form class="card mb-6 grid gap-3 p-5 md:grid-cols-5" method="GET">
    ...
    <select class="input" name="status"><option value="">Todos los estados</option>@foreach(\App\Enums\EnrollmentStatus::cases() as $status)<option value="{{ $status->value }}" @selected(request('status')===$status->value)>{{ ucfirst($status->value) }}</option>@endforeach</select>
...
@forelse($enrollments as $enrollment)<tr><td><a class="font-bold text-teal-700" href="{{ route('students.show',$enrollment->student) }}">{{ $enrollment->student->full_name }}</a></td><td>{{ $enrollment->group->course->name }}<div class="text-xs text-slate-500">{{ $enrollment->group->name }}</div></td><td>{{ $enrollment->status->value }} @if($enrollment->data_status==='needs_review')<span class="badge bg-amber-100 text-amber-800">Requiere revisión</span>@endif</td><td class="text-right">{{ $enrollment->agreed_amount === null ? 'Valor pendiente de definir' : '$'.number_format($enrollment->agreed_amount,0,',','.') }}</td><td class="text-right text-emerald-700">${{ number_format($enrollment->total_paid,0,',','.') }}</td><td class="text-right text-lg font-black {{ $enrollment->balance !== null && $enrollment->balance > 0 ? 'text-rose-600':'text-emerald-700' }}">{{ $enrollment->balance === null ? 'No calculable' : '$'.number_format($enrollment->balance,0,',','.') }}</td><td><a class="btn-primary whitespace-nowrap" href="{{ route('payments.create',$enrollment) }}">Registrar abono</a></td></tr>
