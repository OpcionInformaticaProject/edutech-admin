...
<div class="overflow-x-auto"><table><thead><tr><th>Estudiante</th><th>Curso / grupo</th><th>Fecha</th><th>Estado</th></tr></thead><tbody>@forelse($recentEnrollments as $item)<tr><td><a class="font-semibold hover:text-teal-700" href="{{ route('students.show',$item->student) }}">{{ $item->student->full_name }}</a></td><td>{{ $item->group->course->name }} · {{ $item->group->name }}</td><td>{{ $item->enrollment_date?->format('d/m/Y') ?? 'Sin fecha' }}</td><td><span class="badge">{{ $item->status->label() }}</span></td></tr>@empty<tr><td colspan="4" class="py-10 text-center text-slate-400">Aún no hay matrículas registradas.</td></tr>@endforelse</tbody></table></div></div>
@endsection
